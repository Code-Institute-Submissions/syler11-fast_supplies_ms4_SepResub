...
class TestBagViews(TestCase):
    ...
    def test_add_to_empty_bag_with_size(self):
        ...
        response = self.client.post(f'/bag/add/{product.id}/',
                                    {"quantity": 1, "redirect_url": "view_bag",
                                     "product_size": 1})
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[0]), 'Added size 1 Test Nameto your bag')
    ...
    def test_adjust_bag_quantity_to_two(self):
        ...
        self.assertEqual(str(messages[0]), 'Updated Test Namequantity to '
                         + str(product_id))
    ...
    def test_remove_product_from_bag_with_size(self):
        ...
        self.assertEqual(str(messages[1]),
                         'Removed size 1 Test Namefrom your bag')
